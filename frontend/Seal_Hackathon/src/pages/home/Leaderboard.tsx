...
import { RANKINGS } from './Shared';
import type { Event, RankingEntry } from './Shared';

// Rank badge colours — defined outside component to avoid recreation on every render
...
export const Leaderboard = ({ selectedEvent }: { selectedEvent: Event | null }) => {
  const [filterTrack, setFilterTrack] = useState('All');
  const [searchTerm, setSearchTerm] = useState('');

  // top3 is stable (not affected by filters) — used only for the podium
  const top3 = RANKINGS.slice(0, 3);

  // filteredRankings: full table after applying track filter and team name search
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end max-w-4xl mx-auto pt-6">
        ...
        {/* 2nd — md:order-1 places it on the left on desktop */}
        ...
        {/* 1st — md:-translate-y-6 raises it above its neighbours */}
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-100 pb-6">
          <div className="space-y-2">
            ...
            {/* optional chaining: selectedEvent may be null */}
            <p className="text-sm text-gray-400 font-medium">Verified results for {selectedEvent?.title}</p>
          </div>

          {/* Filters: search by team name + filter by track */}
          <div className="flex items-center gap-3">
            <div className="relative">
              {/* Search icon absolutely positioned over the input */}
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredRankings.length > 0 ? (
                filteredRankings.map((team: RankingEntry) => (
                  // key on team.rank: stable when list order changes
                  <tr key={team.rank} className={`hover:bg-gray-50 transition-colors ${team.rank <= 3 ? 'bg-blue-50/10' : ''}`}>
                    <td className="p-5 text-center">
                      {/* RANK_BADGE_STYLES[team.rank]: falls back to plain style for rank > 3 */}
