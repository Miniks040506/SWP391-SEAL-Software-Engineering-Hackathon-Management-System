          ...
          <section className="bg-white border border-gray-200 rounded-2xl p-8 md:p-10">
            ...
            <div className="relative space-y-12 ml-3 border-l border-gray-100">
              {roadmapSteps.map((step, i) => {
                const isCurrent = i === event.currentPhase - 1 && event.status !== 'Ended';
                return (
                  <div key={step.phase} className="relative pl-10">
                    <div className={`absolute -left-[7.5px] top-1 w-3.5 h-3.5 rounded-full border-4 border-white shadow-md transition-colors ${
                      step.active
                        ? isCurrent ? 'bg-blue-500 ring-2 ring-blue-200 ring-offset-1' : 'bg-blue-300'
                        : 'bg-gray-200'
                    }`} />
