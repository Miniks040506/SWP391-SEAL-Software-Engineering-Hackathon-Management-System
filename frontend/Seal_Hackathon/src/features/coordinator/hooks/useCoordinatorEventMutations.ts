...
const activeRoundApi = (
  USE_MOCK ? mockCoordinatorService.roundApi : roundApi
) as typeof roundApi;
...
export function useCreateEventMutation() {
  ...
  return useMutation({
    mutationFn: (payload: CreateEventRequest) =>
      activeEventApi.createEvent(payload),
    onSuccess: () =>
...
export function useUpdateEventMutation(eventId: UUID) {
  ...
  return useMutation({
    mutationFn: (payload: UpdateEventRequest) =>
      activeEventApi.updateEvent(eventId, payload),
    onSuccess: () => {
...
export function useCreateTrackMutation(eventId: UUID) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: CreateTrackRequest) =>
      activeTrackApi.createTrack(eventId, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({
...
export function useCreateRoundMutation(eventId: UUID) {
  ...
  return useMutation({
    mutationFn: (payload: CreateRoundRequest) =>
      activeRoundApi.createRound(eventId, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({
...
export function useCreatePrizeMutation(eventId: UUID) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: CreatePrizeRequest) =>
      activePrizeApi.createPrize(payload),
    onSuccess: () =>
      queryClient.invalidateQueries({
...
export function useAssignMentorMutation(eventId: UUID) {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: coordinatorEventKeys.mentorAssignments(variables.trackId),
      });
      queryClient.invalidateQueries({
        queryKey: coordinatorEventKeys.tracks(eventId),
      });
...
export function useAssignJudgeMutation(eventId: UUID) {
  ...
  return useMutation({
    mutationFn: ({
      ...
      roundId: UUID;
      payload: AssignJudgeRequest;
    }) => activeRoundApi.assignJudge(roundId, payload),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: coordinatorEventKeys.judgeAssignments(variables.roundId),
      });
      queryClient.invalidateQueries({
        queryKey: coordinatorEventKeys.rounds(eventId),
      });
...
export function useCreateAdvanceRuleMutation(eventId: UUID) {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: coordinatorEventKeys.advanceRules(variables.roundId),
      });
      queryClient.invalidateQueries({
        queryKey: coordinatorEventKeys.rounds(eventId),
      });
